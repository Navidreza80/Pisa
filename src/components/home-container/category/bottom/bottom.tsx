        ...
        <div className="container mx-auto p-4">
            <div className="flex flex-wrap justify-center gap-6 mb-6">
                {cardData.slice(0, 6).map((card) => (
                    <div key={card.id} className="flex-1 bg-black relative rounded-[20px] max-h-[190px] min-w-[389px] max-w-[389px]">
                        <img 
                            src={card.image.src} 
                            alt={card.name}
                        ...
                        <div className="p-4 text-center">
                            <p className="text-lg  absolute bottom-[5%] right-[5%] text-[24px] font-[700] text-white">{card.name}</p>
                        </div>
                    </div>
                ))}
            </div>
