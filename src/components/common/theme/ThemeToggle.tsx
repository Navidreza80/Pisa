  ...
  return (
    <div className={`items-center min-[900px]:flex gap-1 p-1 bg-white/5 rounded-full`}>
      <button
        ...
        className="p-1 xs:p-1.5 rounded-full hover:bg-gray-100 transition-colors"
      >
        {isDarkMode ? (
          <SunIcon className="w-3.5 h-3.5 xs:w-4 xs:h-4 text-gray-700" />
        ) : (
          <MoonIcon className="w-3.5 h-3.5 xs:w-4 xs:h-4 text-gray-700" />
        )}
      </button>
    </div>
  );
